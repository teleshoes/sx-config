...
SilicaFlickable {
    ...
    property real contentTopMargin: Screen.height / 8
    property int statusBarHeight
    property bool twoColumnMode: Screen.sizeCategory <= Screen.Medium
                                 && !lockScreenPage.isPortrait
    ...
    PullDownMenu {
        ...
        enabled: shortcutRepeater.count
                 && !lipstickSettings.lowPowerMode
                 && Lipstick.compositor.systemInitComplete && lockScreen.locked
        opacity: enabled ? 1 : 0
    ...
    ContrastBackground {
        ...
        // phone size cannot fit all in one column, keep the items physically in the same place
        ...
        Clock {
            ...
            property real unlockOpacity: lockScreen.locked ? 1 - (deviceLockItem.locked ? pannableProgress
                                                                                        : lockScreen.progress)
                                                           : 0.0
    ...
    ContrastBackground {
        ...
        x: twoColumnMode ? lockItem.width - width
                           - Math.max(rightIndicator.width + Theme.paddingSmall,
                                      (0.25 * (lockItem.width / 2)) / 2)
                         : (lockItem.width - width) / 2
        y: bottomBackground.y - height - Theme.paddingMedium
        width: twoColumnMode ? 0.75 * (lockItem.width / 2)
                             : Screen.sizeCategory > Screen.Medium ? 4 * Theme.itemSizeExtraLarge
                                                                   : 0.75 * lockItem.width
        ...
        MprisPlayerControls {
            ...
            onItemChanged: {
                if (item) {
                    ...
                    item.buttonSize = Screen.sizeCategory > Screen.Medium ? Theme.iconSizeExtraLarge
                                                                          : Theme.iconSizeLarge
    ...
    ContrastBackground {
        ...
        Column {
            ...
            Loader {
                ...
                width: twoColumnMode ? parent.width / 2 : parent.width
                x: twoColumnMode ? parent.width / 2 : 0
